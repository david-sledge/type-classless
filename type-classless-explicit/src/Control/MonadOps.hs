...
empty :: MonoidLiftOps m -> m a
empty monoidLiftOps = _ident $ _monoidLiftMonoidOps monoidLiftOps

alt :: MonoidLiftOps m -> m a -> m a -> m a
alt monoidLiftOps = _assoc $ _monoidLiftMonoidOps monoidLiftOps
...
pkgAlternativeOps applOps monoidLiftOps =
  AlternativeOps (\ v ->
      let some_v = _liftA2 applOps (:) v (alt monoidLiftOps some_v $ _pure applOps [])
      in some_v) (\ v ->
      let many_v = alt monoidLiftOps (_liftA2 applOps (:) v many_v) $ _pure applOps []
      in many_v) applOps monoidLiftOps

{- class MonadTrans t where
    -- | Lift a computation from the argument monad to the constructed monad.
    lift :: (Monad m) => m a -> t m a -}
type Lift t = forall m a . MonadOps m -> m a -> t m a
